...
export class WindowsWSLClaudeDetector extends PlatformClaudeDetector {
  ...
  private async detectWSLEnvironment(): Promise<WSLInfo> {
    try {
      ...
      console.warn('Failed to detect WSL environment:', error.message)
      return {
        available: false,
        version: 'unknown',
        distributions: []
  ...
  private async detectClaudeInWSL(distro: WSLDistribution): Promise<WSLClaudeResult> {
    ...
    for (const cmd of commands) {
      try {
        ...
        console.warn(`WSL command failed: ${cmd}`, error.message)
        continue
      }
    }
  ...
  private async verifyClaudeInWSL(distroName: string, claudePath: string): Promise<boolean> {
    try {
      ...
      console.warn(`Failed to verify Claude in WSL ${distroName}:`, error.message)
      return false
    }
  }
  ...
  private async getClaudeVersionInWSL(
    ...
    try {
      ...
      console.warn(`Failed to get Claude version in WSL ${distroName}:`, error.message)
    }
