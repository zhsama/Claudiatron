...
export const MCPManager: React.FC<MCPManagerProps> = ({ onBack, className }) => {
  ...
  const loadServers = async () => {
    try {
      ...
      console.log(
        'MCPManager: Server count:',
        Array.isArray(serverList) ? serverList.length : 'not an array'
      )

      // Ensure we have an array
      if (Array.isArray(serverList)) {
        setServers(serverList)
        console.log('MCPManager: Successfully set', serverList.length, 'servers')
      } else {
        console.error('MCPManager: Received non-array response:', serverList)
        setServers([])
      }
    } catch (err) {
  ...
  const handleServerAdded = () => {
    loadServers()
    setToast({ message: t('messages.serverAdded'), type: 'success' })
  ...
  const handleServerRemoved = (name: string) => {
    ...
    setToast({ message: t('messages.serverRemoved', { name }), type: 'success' })
  }
